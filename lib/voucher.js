 ...
 *
 * Format: urn:voucher:txo:<chain>:<txid>:<vout>?key=<privateKey>&amount=<satoshis>
 * Example: urn:voucher:txo:tbtc4:abc123...def:0?key=0123...cdef&amount=1000000
 */
...
export function createVoucher(options) {
  ...
  let uri = `urn:voucher:txo:${chain}:${txid}:${vout}?key=${privateKey}&amount=${amount}`;
