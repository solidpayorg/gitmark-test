 ...
 *
 * Format: txo:<chain>:<txid>:<vout>?amount=<satoshis>&key=<privateKey>
 * Example: txo:tbtc4:abc123...def:0?amount=1000000&key=0123...cdef
 *
...
export function createVoucher(options) {
  ...
  let uri = `txo:${chain}:${txid}:${vout}?amount=${amount}&key=${privateKey}`;
